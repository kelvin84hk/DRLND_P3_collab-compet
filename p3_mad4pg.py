...
class MAD4PG():
    ...
    def __init__(self, state_size, action_size, seed,BUFFER_SIZE = int(1e6),BATCH_SIZE = 64,GAMMA = 0.99,TAU = 1e-3,LR_ACTOR = 1e-4,LR_CRITIC = 1e-4,WEIGHT_DECAY = 0.0001,UPDATE_EVERY = 3,N_step=5):
        ...
        self.mad4pg_agent=[D4PG(state_size, action_size,seed,device),D4PG(state_size, action_size,seed,device)]
    ...
    def step(self, states, actions,rewards, next_states, dones):
        ...
        for agent_index in range(len(self.mad4pg_agent)):
            ...
            if len(self.rewards_queue[agent_index])>=self.N_step:# N-steps return: r= r1+gamma*r2+..+gamma^(t-1)*rt
                ...
                self.mad4pg_agent[agent_index].critic_local.eval()
                with torch.no_grad():
                    Q_expected = self.mad4pg_agent[agent_index].critic_local(state, action)
                self.mad4pg_agent[agent_index].critic_local.train()
                self.mad4pg_agent[agent_index].actor_target.eval()
                with torch.no_grad():
                    action_next = self.mad4pg_agent[agent_index].actor_target(next_state)
                self.mad4pg_agent[agent_index].actor_target.train()
                self.mad4pg_agent[agent_index].critic_target.eval()
                with torch.no_grad():
                    Q_target_next = self.mad4pg_agent[agent_index].critic_target(next_state, action_next)
                    Q_target_next =F.softmax(Q_target_next, dim=1)
                self.mad4pg_agent[agent_index].critic_target.train()
